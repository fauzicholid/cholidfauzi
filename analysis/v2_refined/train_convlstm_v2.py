"""ConvLSTM v2: trained on the improved NDVI+NDMI classification labels
(sequence_raw_v2.pkl), with three input channels (NDVI, brightness, NDMI --
since the v2 labels depend on NDMI, the model needs access to it too, unlike
the original 2-channel NDVI+brightness-only model). Uses the two fixes
already validated in ../experiments/exp2 (reclassify-after-downsample
labels + inverse-frequency class-weighted loss), 5 seeds for robustness.

At full inverse-frequency weighting strength, this configuration reaches
high Non-vegetated recall but collapses precision (User's Accuracy ~13%) --
see train_convlstm_v3.py for the sqrt-dampened remedy used as the paper's
primary refined-ConvLSTM result. This v2 configuration is retained as a
documented cautionary finding about weighting-scheme sensitivity, not
discarded.
"""
...
def run_one(seed):
    ...
    w = counts.sum() / (N_CLASSES * counts)  # full inverse-frequency weighting
...
for seed in range(N_SEEDS):
    ...
    np.save(f"{DATA_DIR}/convlstm_v2_pred2025_seed{seed}.npy", r["pred2025"])

OAs = np.array([r["OA"] for r in all_results])
kappas = np.array([r["kappa"] for r in all_results])
PAs = np.array([r["PA"] for r in all_results])
log("\n" + "=" * 90)
log(f"ConvLSTM v2 (3-channel, label-fix + class-weighted), {N_SEEDS} seeds:")
...
), open(f"{DATA_DIR}/convlstm_v2_results.json", "w"), indent=2)
log("\nSaved convlstm_v2_results.json and per-seed convlstm_v2_pred2025_seed*.npy")
